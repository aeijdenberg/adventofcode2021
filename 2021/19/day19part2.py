...
class Scanner(object):
    def __init__(self, id, b=None, s=None):
        self.id = id
        self.beacons = b if b else set()
        self.scanner = s if s else (0, 0, 0)
    ...
    def each_combo(self):
        for pos_neg in itertools.product(*([[1,-1]]*3)):
            for orders in itertools.permutations(range(3)):
                ...
                ), tuple(pos_neg[i] * self.scanner[orders[i]] for i in range(3)))
    ...
    def overlaps(self, other, target):
        for our_xyz in self.beacons:
            for their_xyz in other.beacons:
                ...
                ), tuple(other.scanner[i] - offsets[i] for i in range(3)))
...
print(max(sum(abs(a.scanner[i] - b.scanner[i]) for i in range(3)) for a, b in itertools.combinations(done_scanners, 2)))
